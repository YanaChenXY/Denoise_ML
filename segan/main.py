...
flags.DEFINE_integer("epoch", 30, "Epochs to train (Def: 150).")
...
flags.DEFINE_integer("denoise_epoch", 5, "Epoch where noise in disc is removed (Def: 5).")
flags.DEFINE_integer("l1_remove_epoch", 150, "Epoch where L1 in G is removed (Def: 150).")
flags.DEFINE_boolean("bias_deconv", True, "Flag to specify if we bias deconvs (Def: False)")
flags.DEFINE_boolean("bias_downconv", True, "flag to specify if we bias downconvs (def: false)")
flags.DEFINE_boolean("bias_D_conv", True, "flag to specify if we bias D_convs (def: false)")
...
flags.DEFINE_string("save_path", "segan_v1_0", "** Path to save out model files. (Def: dwavegan_model).")
flags.DEFINE_string("weights", 'SEGAN-1150', "** Weights file")
...
flags.DEFINE_string("deconv_type", "deconv", "Type of deconv method: deconv or nn_deconv (Def: deconv).")
...
flags.DEFINE_string("synthesis_path", "dwavegan_samples", "Path to save output generated samples. (Def: dwavegan_sam ples).")
flags.DEFINE_string("e2e_dataset", "data/segan.tfrecords", "TFRecords (Def: data/segan.tfrecords.")
flags.DEFINE_string("save_clean_path", "data", "Path to save clean utts")
flags.DEFINE_string("test_wavs", None, "name of test wav (it won't train)")
FLAGS = flags.FLAGS
