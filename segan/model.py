...
class SEGAN(Model):
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        wavbatch, noisybatch = tf.train.shuffle_batch([self.get_wav, self.get_noisy],
            ...
            while not coord.should_stop():
                ...
                if counter % config.save_freq == 0:
                    for d_iter in range(self.disc_updates):
                        _d_opt, _d_sum, d_fk_loss, d_rl_loss = self.sess.run([d_opt,
                                                                              self.d_sum,
                                                                              self.d_fk_losses[0],
                                                                              self.d_rl_losses[0]])
                        if self.d_clip_weights:
                    ...
                    # now G iterations
                    _g_opt, _g_sum,  g_adv_loss,  g_l1_loss = self.sess.run([g_opt,
                                                                             self.g_sum,
                                                                             self.g_adv_losses[0],
                                                                             self.g_l1_losses[0]])
                else:
                ...
                counter += num_devices
                print("counter:", counter, " num_devices:", num_devices, " config.save_freq:", config.save_freq)
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    for m in range(min(20, canvas_w.shape[0])):
                        ...
                        wavfile.write(os.path.join(save_path,
                                                   'sample_{}-{}.wav'.format(counter, m)),
                                      16e3,
                                      de_emph(canvas_w[m],
                                              self.preemph))
                        m_gtruth_path = os.path.join(save_path, 'gtruth_{}.wav'.format(m))
                        if not os.path.exists(m_gtruth_path):
                            print('save_path:',save_path)
                            wavfile.write(os.path.join(save_path, 'gtruth_{}.wav'.format(m)),
                                          16e3,
                                          de_emph(swaves[m],self.preemph))

                            wavfile.write(os.path.join(save_path, 'noisy_{}.wav'.format(m)),
                                          16e3,
                                          de_emph(sample_noisy[m], self.preemph))

                            wavfile.write(os.path.join(save_path, 'dif_{}.wav'.format(m)),
                                          16e3,
