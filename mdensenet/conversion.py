...
def loadAudioFile(filePath, sr=None):
    audio, sampleRate = librosa.load(filePath, sr=sr)
...
def spectrogramToAudioFile(spectrogram, frame_rate, fftWindowSize, phaseIterations=10, phase=None):
    ...
    Wn = cutoff_frequency/frame_rate
...
def handleAudio(filePath, args):
    ...
    audio, sampleRate = loadAudioFile(INPUT_FILE, sr=48000)
...
def pre_matrix(matrix, scale):
    ...
    for time in range(0, ceil(matrix.shape[1] / scale)):
        ...
        for freq in range(0, ceil(matrix.shape[0] / scale)):
            ...
            print(freq * scale, next_freq, time * scale, next_time)
            slices.append(s)
    x = (matrix.shape[0]//scale + 1)*scale
    y = (matrix.shape[1]//scale + 1)*scale
    # slices = np.zeros((x, y))
    #
    # slices[:matrix.shape[0], :matrix.shape[1]] = matrix
    # slices = np.reshape(slices, newshape=[-1, scale, scale])
    return slices, x, y


def reshape_matrix(matrix, pre_shape, new_shape):
    slices = np.zeros(new_shape)
    count = 0
    scale = pre_shape[1]
    matrix = np.squeeze(matrix)
    for time in range(0, ceil(new_shape[1] / scale)):
        if new_shape[1] <= (time + 1) * scale:
            next_time = new_shape[1]
            y = next_time % scale
        else:
            next_time = (time + 1) * scale
            y = scale
        for freq in range(0, ceil(new_shape[0] / scale)):
            if new_shape[0] <= (freq + 1) * scale:
                next_freq = new_shape[0]
                x = next_freq % scale
            else:
                next_freq = (freq + 1) * scale
                x = scale
            slices[freq * scale: next_freq, time * scale: next_time] = matrix[count,:x,:y]
            print(freq * scale, next_freq, time * scale, next_time)
            count += 1
    return slices
